...
#include "Infrastructure/StHbtVertexMultAnalysis.h"
...
StHbtVertexMultAnalysis::StHbtVertexMultAnalysis(const StHbtVertexMultAnalysis& a) : StHbtAnalysis() {
  ...
  mOverFlowMult = 0; 
  if (mMixingBuffer) delete mMixingBuffer;
  mPicoEventCollectionVectorHideAway = new StHbtPicoEventCollectionVectorHideAway(mVertexZBins,mVertexZ[0],mVertexZ[1],mMultBins,mMult[0],mMult[1]);
...
void StHbtVertexMultAnalysis::ProcessEvent(const StHbtEvent* hbtEvent) {
  ...
  //if mult=0, event will not be processed.
  //but where does it removes these events? ans: no mMixingBuffer 
