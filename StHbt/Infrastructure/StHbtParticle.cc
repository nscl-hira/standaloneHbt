...
int TpcLocalTransform(StThreeVectorD &xgl, int &iSector, int &iPadrow, float &xlocal, double &ttPhi);

//_____________________
StHbtParticle::StHbtParticle() : mTrack(0), mV0(0), mKink(0), mHiddenInfo(0) { /* no-op for default */ }
//_____________________
...
StHbtParticle::StHbtParticle(const StHbtTrack *const hbtTrack, const double &mass) :
	...
	CalculateTpcExitAndEntrancePoints(
		&mHelix, &mPrimaryVertex, &mSecondaryVertex, &mNominalTpcEntrancePoint, &mNominalTpcExitPoint,
		&mNominalPosSample[0], &mZ[0], &mU[0], &mSect[0]
	);
...
StHbtParticle::StHbtParticle(const StHbtV0 *const hbtV0, const double &mass) :
	...
	CalculateTpcExitAndEntrancePoints(
		&mHelixV0Pos, &mPrimaryVertex, &mSecondaryVertex, &mTpcV0PosEntrancePoint, &mTpcV0PosExitPoint,
		&mNominalPosSample[0], &mZ[0], &mU[0], &mSect[0]
	);
	...
	CalculateTpcExitAndEntrancePoints(
		&mHelixV0Neg, &mPrimaryVertex, &mSecondaryVertex, &mTpcV0NegEntrancePoint, &mTpcV0NegExitPoint,
		&mTpcV0NegPosSample[0], &mV0NegZ[0], &mV0NegU[0], &mV0NegSect[0]
	);
...
void StHbtParticle::CalculateTpcExitAndEntrancePoints(
	StPhysicalHelixD *tHelix, StHbtThreeVector *PrimVert, StHbtThreeVector *SecVert,
	StHbtThreeVector *tmpTpcEntrancePoint, StHbtThreeVector *tmpTpcExitPoint, StHbtThreeVector *tmpPosSample,
	...
	if (endLength < 0.0)
		cout << "StHbtParticle::CalculateTpcExitAndEntrancePoints(): "
	...
	*tmpTpcEntrancePoint = hel.at(sideLength);
	...
	if (TMath::IsNaN(tmpTpcEntrancePoint->x()) || TMath::IsNaN(tmpTpcEntrancePoint->y()) ||
		TMath::IsNaN(tmpTpcEntrancePoint->z())) {
	...
	if (TMath::IsNaN(tmpTpcExitPoint->x()) || TMath::IsNaN(tmpTpcExitPoint->y()) ||
		TMath::IsNaN(tmpTpcExitPoint->z())) {
	...
	//   if (TMath::IsNaN(tmpTpcExitPoint->x())) *tmpTpcExitPoint = StHbtThreeVector(-9999.,-9999.,-9999);
	//   if (TMath::IsNaN(tmpTpcEntrancetPoint->x())) *tmpTpcEntrancePoint = StHbtThreeVector(-9999.,-9999.,-9999);
	...
	sideLength = (candidates.first > 0) ? candidates.first : candidates.second;
	while (irad < 11 && !TMath::IsNaN(sideLength)) {
		float radius = 50.0 + irad * 15.0;
		candidates = hel.pathLength(radius);
		sideLength = (candidates.first > 0) ? candidates.first : candidates.second;
		tmpPosSample[irad] = hel.at(sideLength);
		if (TMath::IsNaN(tmpPosSample[irad].x()) || TMath::IsNaN(tmpPosSample[irad].y()) ||
			TMath::IsNaN(tmpPosSample[irad].z())) {
	...
	static float tRowRadius[45] = {60,		64.8,	 69.6,	  74.4,	   79.2,	84,		 88.8,	  93.6,	   98.8,
								   104,		109.2,	 114.4,	  119.6,   127.195, 129.195, 131.195, 133.195, 135.195,
								   137.195, 139.195, 141.195, 143.195, 145.195, 147.195, 149.195, 151.195, 153.195,
								   155.195, 157.195, 159.195, 161.195, 163.195, 165.195, 167.195, 169.195, 171.195,
								   173.195, 175.195, 177.195, 179.195, 181.195, 183.195, 185.195, 187.195, 189.195};
	...
	// end test
	while (ti < 45 && !TMath::IsNaN(tLength)) {
		candidates = hel.pathLength(tRowRadius[ti]);
		tLength = (candidates.first > 0) ? candidates.first : candidates.second;
		if (TMath::IsNaN(tLength)) {
...
//_____________________
const StHbtThreeVector &StHbtParticle::TpcV0PosExitPoint() const { return mTpcV0PosExitPoint; }
//_____________________
const StHbtThreeVector &StHbtParticle::TpcV0PosEntrancePoint() const { return mTpcV0PosEntrancePoint; }
//______________________
const StHbtThreeVector &StHbtParticle::TpcV0NegExitPoint() const { return mTpcV0NegExitPoint; }
//_____________________
const StHbtThreeVector &StHbtParticle::TpcV0NegEntrancePoint() const { return mTpcV0NegEntrancePoint; }
//______________________
