 ...
 * fixed small bug in StHbtAnalysis associated with the new feature to require ParticleCollections to have some minimum
 *number of particles
 *
...
#include "Infrastructure/StHbtAnalysis.h"
...
void FillHbtParticleCollection(
	...
	case hbtV0: // cut is cutting on V0s
	{
		...
		// this following "for" loop is identical to the one above, but because of scoping, I can's see how to avoid
		// repitition...
		for (pIter = startLoop; pIter != endLoop; pIter++) {
			pParticle = *pIter;
...
StHbtAnalysis::StHbtAnalysis(const StHbtAnalysis &a) : StHbtBaseAnalysis() {
	...
	for (iter = a.mCorrFctnCollection->begin(); iter != a.mCorrFctnCollection->end(); iter++) {
		...
		if (fctn)
			...
			cout << " StHbtAnalysis::StHbtAnalysis(const StHbtAnalysis& a) - correlation function not found " << endl;
	}
	...
	if (tmpPassEvent) {
		...
		if (!(AnalyzeIdenticalParticles()))
			FillHbtParticleCollection(
				mSecondParticleCut, (StHbtEvent *)hbtEvent, mPicoEvent->SecondParticleCollection()
			);
		...
		if ((mPicoEvent->FirstParticleCollection()->size() >= mMinSizePartCollection) &&
			...
			if (AnalyzeIdenticalParticles()) {
				...
				MakePairs("real", mPicoEvent->FirstParticleCollection(), mPicoEvent->SecondParticleCollection());
			}
			...
			for (mPicoEventIter = MixingBuffer()->begin(); mPicoEventIter != MixingBuffer()->end(); mPicoEventIter++) {
				...
				if (AnalyzeIdenticalParticles()) {
					MakePairs("mixed", mPicoEvent->FirstParticleCollection(), storedEvent->FirstParticleCollection());
				} else {
...
//_________________________
void StHbtAnalysis::MakePairs(
	const char *typeIn, StHbtParticleCollection *partCollection1, StHbtParticleCollection *partCollection2
) {
...
//_________________________
void StHbtAnalysis::AddEventProcessed() { mNeventsProcessed++; }
