...
#include "Base/StHbtXiCut.h"
#include "Base/StHbtKinkCut.h"
#include "PhysicalConstants.h"
#include "SystemOfUnits.h"

// Mike removed all of the StHbtTTree stuff here 21apr2006 - it was not used for a long time.
...
void StHbtEvent::Rotate3D(const double theta, const double phi){                
  ...
  StHbtThreeVector o;                                                           
  double c;       
  mReactionPlane[0] += theta;                                                   
  mReactionPlanePtWgt[0] += theta;                                              
  cout << " StHbtEvent::RotateZ(const double theta, const double phi ) =" << theta <<" , "<<phi<<") " << " rad    ";                                           
  cout << "theta  = "<<theta / degree << " deg ";                               
  cout << "\t phi = "<<phi / degree << " deg " << endl;                         
                                                                                
  for (iter=mTrackCollection->begin();iter!=mTrackCollection->end();iter++)     
    {                                                                             
      p = (*iter)->P();    p.rotateZ(theta);  p.rotateX(phi); (*iter)->SetP(p); 
      p= (*iter)->Helix().momentum(mMagneticField*kilogauss);                   
      o= (*iter)->Helix().origin();                                             
      p.rotateZ(theta);                                                         
      p.rotateX(phi);                                                           
...
void StHbtEvent::SetEtrans(const float& etrans)  {  mEtrans = etrans;}
float StHbtEvent::GetEtrans() const {  return mEtrans;}

void StHbtEvent::SetBhat(const float& b){bhat=b;}
float StHbtEvent::GetBhat() const {return bhat;}

void StHbtEvent::SetuBallDS_TRG(const float& b){uBallDS_TRG=b;}
float StHbtEvent::GetuBallDS_TRG() const {return uBallDS_TRG;}

void StHbtEvent::SetuBallHira_TRG(const float& b){uBallHira_TRG=b;}
float StHbtEvent::GetuBallHira_TRG() const {return uBallHira_TRG;}

void StHbtEvent::SetuBallNW_TRG(const float& b){uBallNW_TRG=b;}
float StHbtEvent::GetuBallNW_TRG() const {return uBallNW_TRG;}
