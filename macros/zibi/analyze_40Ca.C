...
void analyze_40Ca( const Int_t nevents    = 1000000,
		   const Char_t *outputfile="output.root",
		   const Char_t *list = "lists/40Ca.list",
		   const int rap = 0,
		   ...
		   const double pTmax = 10000.)
{
  gSystem->Load("load.C");
    ...
    //    HiRAReader* Reader = new HiRAReader(0,"",list,"root",nevents,beta);
	...
	switch (rap)
	  ...
	  case 1 : rapMin = -0.10; rapMax = -0.05; break;
	...
	for (int centrality=0; centrality<1; centrality++)
	  {
		...
		switch (mom)
		  ...
		  case 3 : PLABmin = 740; PLABmax = 900; break;
		  }
		...
		for (int theta=0; theta<1; theta++)
		  ...
		  switch(theta)
		    ...
		    case 1 : thetaLABmin = 0; thetaLABmax = 26; break;
		    case 2 : thetaLABmin = 26; thetaLABmax = 33; break;
		    case 3 : thetaLABmin = 33; thetaLABmax = 90; break;
		    }
		 ...
		 eventPass[rap][centrality][mom][theta] = new HiRAEventCutMonitor();
		  ...
		  char* chartest = (char*)"ab";
		  // 2) Now the Correlation Functions - instantiate+configure+plug
		  QinvCF[rap][centrality][mom][theta] = new QinvCorrFctn_MomCons((char*)("Qinv"),35,0.0,140);
		  //		  QinvCF[rap][centrality][mom][theta] = new QinvCorrFctn_MomCons((char*)(analname+"Qinv").Data(),35,0.0,140);
		  //QinvCF[rap][centrality][mom][theta] = new QinvCorrFctn_MomCons(chartest,35,0.0,140);
		  anal[rap][centrality][mom][theta]->AddCorrFctn(QinvCF[rap][centrality][mom][theta]);  
		  
		  /*
		  MinvCF[rap][centrality][mom][theta] = new MinvCorrFctn((analname+"Minv").Data(),50,1860,1910);
		  anal[rap][centrality][mom][theta]->AddCorrFctn(MinvCF[rap][centrality][mom][theta]);  

		  KstarCF[rap][centrality][mom][theta] = new KstarCorrFctn_MomCons((analname+"Kstar").Data(),70,0.0,280);
		  anal[rap][centrality][mom][theta]->AddCorrFctn(KstarCF[rap][centrality][mom][theta]);  

		  LCMSCF[rap][centrality][mom][theta] = new BPLCMSFrame3DCorrFctn_MomCons((analname+"LCMSCF").Data(),70,0.0,280);
		  anal[rap][centrality][mom][theta]->AddCorrFctn(LCMSCF[rap][centrality][mom][theta]);

		  PRCF[rap][centrality][mom][theta] = new PRFrame3DCorrFctn_MomCons((analname+"PRCF").Data(),70,0.0,280);
		  anal[rap][centrality][mom][theta]->AddCorrFctn(PRCF[rap][centrality][mom][theta]);

		  LabCF[rap][centrality][mom][theta] = new LabFrame3DCorrFctn_MomCons((analname+"LabCF").Data(),70,0.0,280);
		  anal[rap][centrality][mom][theta]->AddCorrFctn(LabCF[rap][centrality][mom][theta]);
		  
		  QtotLCMS_CF[rap][centrality][mom][theta] = new QtotCosThetaPhi3DcorrFctn_MomCons((analname+"QtotLCMS_CF").Data(),70,0.0,280.,12,-1.,1.,12,0,2*TMath::Pi());
		  anal[rap][centrality][mom][theta]->AddCorrFctn(QtotLCMS_CF[rap][centrality][mom][theta]);  
		  
		  QtotLab_CF[rap][centrality][mom][theta] = new LabFrameQtotCosThetaPhi3DcorrFctn_MomCons((analname+"QtotLab_CF").Data(),70,0.0,280.,12,-1.,1.,12,0,2*TMath::Pi());
		  anal[rap][centrality][mom][theta]->AddCorrFctn(QtotLab_CF[rap][centrality][mom][theta]);  
		  
		  QtotPR_CF[rap][centrality][mom][theta] = new PRFrameQtotCosThetaPhi3DcorrFctn_MomCons((analname+"QtotPR_CF").Data(),70,0.0,280.,12,-1.,1.,12,0,2*TMath::Pi());
		  anal[rap][centrality][mom][theta]->AddCorrFctn(QtotPR_CF[rap][centrality][mom][theta]);  
		  */		  
    ...
    // ----------------- process the events
    if (TheManager->Init()) cout << "Problem? - non-zero initialization value\n";
