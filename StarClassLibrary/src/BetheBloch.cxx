...
// double BetheBloch::operator() (double betagamma){
//     return (double) operator()((double) betagamma);
// }
double BetheBloch::Sirrf(double Poverm, double Length, int k) {
  double Scale2keV = 1.67180; // scale to get  2.40 keV/cm at b*g = 4
  double par[7] = {
  ...
  double poverm = Poverm;
  if (poverm > 527.5) poverm = 527.5;
  double beta2inv = 1. + 1./(poverm*poverm);
  double gamma  = sqrt(poverm*poverm + 1);
  double Lpoverm = log(poverm);
  double K      = 0.307075e+3;// keV/g cm^2 
  double A      = 38.691;
  double Z      = 17.436;
  double rho    = 1.5607e-03;//  0.9*0.00166+0.1*0.000667
  double I      = par[1]; //15.8e-6; //15.8; // MeV for Ar, 13.1e-6 MeV for CH4
  double m      =   0.510998902;// MeV electron Mass
  double pim    = 139.570180;// MeV pion Mass
  double Delta;
  double M = pim;
  if (k) {M = m;   Delta = par[6];}
  else   {M = pim; Delta = par[2];}
  double r = m/M;
  double Tmax = 2*m*poverm*poverm/(1. + r*(2*gamma + r));
  double Tupper = Tmax;
  double si = K*Z/A*rho/2*beta2inv*
    (log(2*m*poverm*poverm*Tupper/(I*I)) 
     - (1 + Tupper/Tmax)/beta2inv - Delta);
  if (si <= 0) si = 1.e-12;
  double value = par[0] + log(si) + 
    Lpoverm*(par[3] + Lpoverm*(par[4] + Lpoverm*par[5]));
  double sirrf =  exp(value)*Scale2keV;
   const int Nm = 12;
   double coeff[Nm] = { 
...
double  BetheBloch::Girrf(double poverm,  double Tmin, int k) {
