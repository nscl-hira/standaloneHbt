...
int drand48_iterate(unsigned short int xsubi[3], struct drand48_data *buffer) {
	...
	if (sizeof(unsigned short int) == 2) {
		...
		a = ((ULONGLONG)buffer->a[2] << 32 | (ULONGLONG)buffer->a[1] << 16 | buffer->a[0]);
...
void srand48(long seedval) { (void)srand48_r(seedval, &libc_drand48_data); }
...
#if USHRT_MAX == 65535
	*result =
		((double)xsubi[2] / ((ULONGLONG)1 << 48) + (double)xsubi[1] / ((ULONGLONG)1 << 32) +
