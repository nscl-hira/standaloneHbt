...
HepDouble RandFlat::operator()() { return fire(); }

void RandFlat::shootArray(const HepInt size, HepDouble *vect, HepDouble lx, HepDouble dx) {
	HepInt i;
...
void RandFlat::shootArray(HepRandomEngine *anEngine, const HepInt size, HepDouble *vect, HepDouble lx, HepDouble dx) {
	for (int i = 0; i < size; ++i)
		vect[i] = shoot(anEngine, lx, dx);
}

#ifndef ST_NO_TEMPLATE_DEF_ARGS
void RandFlat::shootArray(HepRandomEngine *anEngine, vector<HepDouble> &vec, HepDouble lx, HepDouble dx)
#else
void RandFlat::shootArray(
	HepRandomEngine *anEngine, vector<HepDouble, allocator<HepDouble>> &vec, HepDouble lx, HepDouble dx
)
#endif
{
	for (unsigned int i = 0; i < vec.size(); ++i)
...
void RandFlat::fireArray(const HepInt size, HepDouble *vect, HepDouble lx, HepDouble dx) {
	HepInt i;
