/***************************************************************************
 *
 * $Id: HiRAPairCut.h,v 1.5 2000/03/23 22:57:28 laue Exp $
 *
...
#ifndef HiRAPairCut_hh
#define HiRAPairCut_hh

#include "Base/StHbtPairCut.h"

class HiRAPairCut : public StHbtPairCut {
public:
	HiRAPairCut();
	HiRAPairCut(const HiRAPairCut &);
	//~HiRAPairCut();

	virtual bool Pass(const StHbtPair *);
	virtual StHbtString Report();
	HiRAPairCut *Clone();
	void SetPCMS(double, double);
	void SetPLab(double, double);
	void SetkT(double, double);
	void SetqInv(double, double);
...
#ifdef __ROOT__
	ClassDef(HiRAPairCut, 1);
#endif
};

inline HiRAPairCut::HiRAPairCut(const HiRAPairCut &c) : StHbtPairCut(c) {
	mNPairsPassed = 0;
	mNPairsFailed = 0;
	pcmsmin = 0;
	pcmsmax = 1000000;
...
inline HiRAPairCut *HiRAPairCut::Clone() {
	HiRAPairCut *c = new HiRAPairCut(*this);
	return c;
}
